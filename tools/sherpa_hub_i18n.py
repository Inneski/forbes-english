...
LANGS['de'] = dict(
    ...
    lgColour='farbig &mdash; antippen und los', lgGrey='grau &mdash; öffnet sich, wenn du sein Lager abschließt',
    ...
    proP='<strong>Vier Lektionen sind kostenlos, ohne Anmeldung:</strong> Lager eins und zwei, das Passiv, das Lager eins öffnet, und <em>used to</em>. Pro öffnet die anderen einundzwanzig und jede weitere Lektion auf der Website.',
    ...
    gDesc='<b>Der Abstieg &middot; Passiv.</b> Das, womit etwas geschieht, steht vorn, und wer handelt, kann wegfallen.',
    ...
    tailAll='alles ist offen.', tailWhole='der ganze Berg ist offen, beide Seiten.',
...
LANGS['es'] = dict(
    ...
    darkP='La pasiva del present perfect continuous, del past perfect continuous, del future continuous y del future perfect continuous se puede formar &mdash;HAS BEEN BEING CLEANED, WILL BE BEING CLEANED&mdash;, pero los hablantes nativos las evitan todas y dan la vuelta a la frase. Ahí no hay nada que aprender, así que no hay campamento.',
    teachH='¿Das clase?',
    ...
    proP='<strong>Cuatro lecciones son gratis, sin registrarte:</strong> los campamentos uno y dos, la pasiva que abre el campamento uno y <em>used to</em>. Pro abre las otras veintiuna, y todas las demás lecciones del sitio.',
    ...
    startFree='Empezar en el campamento uno &middot; gratis', backMap='Volver al mapa',
    carryOn='Sigue: campamento {n} &middot; {name}', headDown='Baja: {name}', offRoute='Fuera de la ruta: {name}',
    again='Súbelo otra vez: campamento uno',
...
LANGS['fr'] = dict(
    langGroup='Langue', sub='une route qui monte à travers les temps', eyebrow="Carte de l'itinéraire",
    h1='Treize camps, deux faces',
    lead="La montée, à la voix active, classée selon la fréquence à laquelle tu utiliseras vraiment chaque temps. La descente, à la voix passive : les treize mêmes camps dans l'autre sens &mdash; dont neuf avec un passif qui vaut la peine d'être appris.",
    ...
    mapHow="La montée n'a pas de verrou : commence au camp dont tu as besoin. Chaque étape de la descente s'ouvre quand tu termines le camp dont elle vient ; en attendant, son repère est gris.",
    guideKick='Nouveau ici ?', guideLine="Deux minutes pour comprendre ce qu'est un temps, ce que veulent dire actif et passif, et pourquoi les camps viennent dans cet ordre.",
    guideBtn="Qu'est-ce que je regarde ?", ascH='La montée', ascSub="voix active &middot; du camp de base au sommet",
    descH='La descente', descSub='voix passive &middot; du sommet à la vallée',
    ascAria="La montée : une montagne avec treize repères de couleur sur un chemin du camp de base au sommet, un pour chaque temps à la voix active, et deux nuages pour used to et be used to. Toutes les étapes sont aussi listées sous la carte.",
    descAria="La descente : la même montagne la nuit, avec un losange pour chaque temps à la voix passive, du sommet vers le bas. Neuf sont des leçons, chacune s'ouvre quand tu termines son camp ; quatre restent sombres parce que personne n'utilise ces passifs. La lune, c'est le causatif. Toutes les étapes sont aussi listées sous la carte.",
    ...
    descList='La descente &middot; voix passive', descListSub="chacune s'ouvre quand tu termines le camp dont elle vient",
    offH="Hors de l'itinéraire", offSub='ce ne sont pas des temps, mais ils discutent avec un temps',
    ...
    darkP="Le passif du present perfect continuous, du past perfect continuous, du future continuous et du future perfect continuous peut se construire &mdash; HAS BEEN BEING CLEANED, WILL BE BEING CLEANED &mdash; mais les anglophones les évitent tous et reformulent la phrase. Il n'y a rien à apprendre là, donc il n'y a pas de camp.",
    teachH='Tu enseignes ?',
    ...
    proP="<strong>Quatre leçons sont gratuites, sans inscription :</strong> les camps un et deux, le passif qu'ouvre le camp un, et <em>used to</em>. Pro ouvre les vingt et une autres, et toutes les autres leçons du site.",
    plans='Formules et tarifs',
    gKick='Avant de monter', gTitle="Ce que tu regardes", gClose='Fermer le guide',
    g1H='Un temps, c\'est un moment et une forme',
    g1P="Chaque temps place une action dans le passé, le présent ou le futur, et lui donne l'une de quatre formes : <b>simple</b> (le fait tout court), <b>continuous</b> (en cours), <b>perfect</b> (un regard en arrière depuis un point plus tardif : maintenant, ou un moment du passé ou du futur) ou <b>perfect continuous</b> (en cours jusqu'à ce point). Trois moments, quatre formes : douze des treize camps. Chaque forme ci-dessous est &laquo;&nbsp;she&nbsp;&raquo; + CLIMB, et le numéro est son camp.",
    gCaption='Les douze temps par moment et par forme ; chacun renvoie à son camp.',
    gGoing="Celui qui reste, le camp 5, est un futur qu'on voit déjà venir.",
    g2H='Deux faces : actif et passif',
    ...
    g2P="Le même temps, retourné : le passif, c'est BE à ce temps + le PAST PARTICIPLE. Voilà pourquoi chaque étape de la descente est la jumelle d'un camp de la montée, et s'ouvre quand tu termines ce camp.",
    g3H='Pourquoi cet ordre',
    g3P="Les camps montent dans l'ordre où tu en auras besoin, pas dans celui des livres de grammaire. Les <b>camps de base</b> sont les temps que tu utilises tous les jours ; <b>l'arête</b> est fréquente mais demande plus de soin ; l'<b>assaut final</b> est rare et surtout écrit. Quatre passifs n'ont pas de camp : on peut les construire &mdash; HAS BEEN BEING CLEANED &mdash; mais personne ne les dit.",
    g3Map="Sur la carte, un cercle est actif, un losange est passif, et le gris veut dire pas encore ouvert ; un losange pâle marqué &laquo;&nbsp;no camp&nbsp;&raquo; ne s'ouvre jamais. Les nuages sont &laquo;&nbsp;used to&nbsp;&raquo; et &laquo;&nbsp;be used to&nbsp;&raquo; ; la lune, c'est le causatif.",
    startFree='Commencer au camp un &middot; gratuit', backMap='Retour à la carte',
    carryOn='Continue : camp {n} &middot; {name}', headDown='Descends : {name}', offRoute="Hors de l'itinéraire : {name}",
    again='Remonte : camp un',
    ...
    tailDefault="neuf des camps ouvrent un passif ; <em>used to</em> s'ouvre après le camp 2 ou 3, <em>be used to</em> après le camp 2, et le causatif après ta première étape de descente.",
    teachOff="J'enseigne ce cours &mdash; tout ouvrir", teachOn='Verrous levés &mdash; tout est ouvert',
    teachWhy="Les apprenants ouvrent chaque passif, et les trois leçons hors de l'itinéraire, en terminant le camp dont ils viennent. Les enseignants, et ceux qui sont pressés, peuvent tout ouvrir.",
    pair='le camp {n} ({tense})', whyDescent="Termine {pair} et celle-ci s'ouvre.",
    whyUsedTo="Termine le camp 2 ou le camp 3 &mdash; used to est une habitude du present simple, passée au passé.",
    ...
    srLocked=' (verrouillé : {why})', srDone=' (terminé)', fromOpen='du camp {n}', fromLocked='après le camp {n}',
    hintNoCamp="{label} : pas de camp ici &mdash; lis la note sous les listes.", hintLocked='{label} : {why}',
)

LANGS['it'] = dict(
    langGroup='Lingua', sub='un percorso in salita tra i tempi verbali', eyebrow='Mappa del percorso',
    h1='Tredici campi, due versanti',
    lead="La salita, in forma attiva, in ordine di quanto spesso userai davvero ogni tempo. La discesa, in forma passiva: gli stessi tredici campi al contrario &mdash; nove dei quali con un passivo che vale la pena imparare.",
    ...
    offH='Fuori percorso', offSub='non sono tempi, ma discutono con uno',
    ...
    darkP="Il passivo del present perfect continuous, del past perfect continuous, del future continuous e del future perfect continuous si può costruire &mdash; HAS BEEN BEING CLEANED, WILL BE BEING CLEANED &mdash; ma i madrelingua li evitano tutti e riformulano la frase. Lì non c'è niente da imparare, quindi non c'è un campo.",
    teachH='Insegni?',
    teachP="L'interruttore sotto la mappa toglie in un colpo tutti i lucchetti di avanzamento in questo browser &mdash; per lo schermo della classe, o per una classe che ha già fatto la salita. Le lezioni Pro richiedono sempre Pro. <a href=\"block-camp.html\">Block Camp</a> sale lungo nove di questi tempi in un mondo fatto di blocchi.",
    proP="<strong>Quattro lezioni sono gratis, senza registrazione:</strong> i campi uno e due, il passivo che apre il campo uno e <em>used to</em>. Pro apre le altre ventuno, e tutte le altre lezioni del sito.",
...
LANGS['pt'] = dict(
    ...
    lead="A subida, na voz ativa, em ordem de quanto você vai usar de verdade cada tempo. A descida, na voz passiva: os mesmos treze acampamentos ao contrário &mdash; nove deles com uma passiva que vale a pena aprender.",
    ...
    lgColour='colorido &mdash; toque para ir', lgGrey='cinza &mdash; abre quando você termina o acampamento',
    ...
    offH='Fora da trilha', offSub='não são tempos, mas discutem com um',
    ...
    g3Map="No mapa, um círculo é ativa, um losango é passiva, e o cinza quer dizer ainda não aberto; um losango apagado marcado &ldquo;no camp&rdquo; nunca se abre. As nuvens são &ldquo;used to&rdquo; e &ldquo;be used to&rdquo;; a lua é o causativo.",
    ...
    pair='o acampamento {n} ({tense})', whyDescent='Termine {pair} e esta se abre.',
...
LANGS['ru'] = dict(
    langGroup='Язык', sub='маршрут вверх по временам', eyebrow='Карта маршрута',
    h1='Тринадцать лагерей, два склона',
    lead='Подъём — в активном залоге, по порядку того, как часто тебе на самом деле понадобится каждое время. Спуск — в пассивном: те же тринадцать лагерей в обратном порядке &mdash; у девяти из них есть пассив, который стоит выучить.',
    countsAria='Что есть на маршруте', cUp='лагерей на подъёме', cDown='пассивов на спуске', cOff='в стороне от маршрута', cFree='бесплатно',
    everyStop='Все остановки', mapKick='Карта', mapH2='Вверх — в активе, вниз — в пассиве',
    mapHow='На подъёме нет замков: начинай с того лагеря, который тебе нужен. Каждая остановка на спуске открывается, когда ты проходишь лагерь, из которого она выходит; до этого её отметка серая.',
    ...
    guideBtn='Что я вижу?', ascH='Подъём', ascSub='активный залог &middot; от базового лагеря до вершины',
    descH='Спуск', descSub='пассивный залог &middot; от вершины к подножию',
    ascAria='Подъём: гора с тринадцатью цветными отметками лагерей на тропе от базового лагеря до вершины, по одной на каждое время в активном залоге, и два облака для used to и be used to. Все остановки перечислены и под картой.',
    ...
    descList='Спуск &middot; пассивный залог', descListSub='каждая открывается, когда ты проходишь лагерь, из которого она выходит',
    ...
    darkP='Пассив от present perfect continuous, past perfect continuous, future continuous и future perfect continuous построить можно &mdash; HAS BEEN BEING CLEANED, WILL BE BEING CLEANED &mdash; но носители языка избегают их все и перестраивают фразу. Учить там нечего, поэтому и лагеря там нет.',
    teachH='Ты преподаёшь?',
    teachP='Переключатель под картой снимает все замки прогресса в этом браузере сразу &mdash; для экрана в классе или для группы, которая уже прошла подъём. Для уроков Pro по-прежнему нужен Pro. <a href="block-camp.html">Block Camp</a> проходит девять из этих времён в мире из блоков.',
    proP='<strong>Четыре урока бесплатны, без регистрации:</strong> лагеря один и два, пассив, который открывает лагерь один, и <em>used to</em>. Pro открывает остальные двадцать один и все остальные уроки на сайте.',
    plans='Тарифы и цены',
    gKick='Перед подъёмом', gTitle='Что ты видишь', gClose='Закрыть подсказку',
    g1H='Время — это момент и форма',
    g1P='Каждое время помещает действие в прошлое, настоящее или будущее и придаёт ему одну из четырёх форм: <b>simple</b> (просто факт), <b>continuous</b> (в процессе), <b>perfect</b> (взгляд назад из более поздней точки: сейчас, или момент в прошлом или будущем) или <b>perfect continuous</b> (в процессе вплоть до этой точки). Три момента, четыре формы: двенадцать из тринадцати лагерей. Каждая форма ниже — это &laquo;she&raquo; + CLIMB, а число — её лагерь.',
    ...
    g2P='То же время, перевёрнутое: пассив — это BE в этом времени + PAST PARTICIPLE. Поэтому каждая остановка на спуске — близнец лагеря на подъёме и открывается, когда ты проходишь этот лагерь.',
    g3H='Почему такой порядок',
    g3P='Лагеря идут в том порядке, в каком они тебе понадобятся, а не как в учебниках грамматики. <b>Базовые лагеря</b> — это времена, которыми ты пользуешься каждый день; <b>гребень</b> встречается часто, но требует внимания; <b>штурм вершины</b> редок и в основном письменный. У четырёх пассивов нет лагеря: их можно построить &mdash; HAS BEEN BEING CLEANED &mdash; но никто так не говорит.',
    ...
    again='Подняться снова: лагерь один',
    ...
    tailDefault='девять лагерей открывают пассив; <em>used to</em> открывается после лагеря 2 или 3, <em>be used to</em> — после лагеря 2, каузатив — после твоей первой остановки на спуске.',
    teachOff='Я преподаю &mdash; открыть всё', teachOn='Замки сняты &mdash; всё открыто',
    teachWhy='Ученики открывают каждый пассив и три урока в стороне от маршрута, проходя лагерь, из которого они выходят. Преподаватели и все, кто спешит, могут открыть всё сразу.',
    pair='лагерь {n} ({tense})', whyDescent='Пройди {pair}, и эта остановка откроется.',
    whyUsedTo='Пройди лагерь 2 или лагерь 3 &mdash; used to — это привычка из present simple, перенесённая в прошлое.',
    whyBeUsedTo='Пройди лагерь 2 (present simple) &mdash; be used to — это состояние, а не событие.',
    whyCausative='Пройди любую остановку на спуске &mdash; каузатив — это пассив в шляпе.',
    srLocked=' (закрыто: {why})', srDone=' (пройдено)', fromOpen='из лагеря {n}', fromLocked='после лагеря {n}',
    hintNoCamp='{label}: здесь нет лагеря &mdash; смотри заметку под списками.', hintLocked='{label}: {why}',
)

LANGS['ar'] = dict(
    langGroup='اللغة', sub='طريق صاعد عبر الأزمنة', eyebrow='خريطة الطريق',
    h1='ثلاثة عشر مخيمًا، ووجهان',
    lead='الصعود بصيغة المبني للمعلوم، مرتّبًا بحسب عدد المرات التي ستحتاج فيها فعلًا إلى كل زمن. والنزول بصيغة المبني للمجهول: المخيمات الثلاثة عشر نفسها بترتيب معكوس &mdash; تسعة منها فيها مبني للمجهول يستحق التعلّم.',
    countsAria='ما الموجود على الطريق', cUp='مخيمًا في الصعود', cDown='صيغ مجهول في النزول', cOff='خارج الطريق', cFree='مجانية',
    ...
    darkP='يمكن بناء المبني للمجهول من present perfect continuous و past perfect continuous و future continuous و future perfect continuous &mdash; HAS BEEN BEING CLEANED، WILL BE BEING CLEANED &mdash; لكن أهل اللغة يتجنّبونها كلها ويعيدون صياغة الجملة. لا شيء هناك يُتعلَّم، ولذلك لا مخيم هناك.',
    teachH='هل تُدرِّس؟',
    ...
    proP='<strong>أربعة دروس مجانية، بلا تسجيل:</strong> المخيمان الأول والثاني، والمبني للمجهول الذي يفتحه المخيم الأول، و<em>used to</em>. أما Pro فيفتح الواحد والعشرين الباقية، وكل الدروس الأخرى في الموقع.',
    plans='الخطط والأسعار',
    gKick='قبل أن تصعد', gTitle='ما الذي تراه', gClose='إغلاق الدليل',
    g1H='الزمن وقتٌ وشكل',
    g1P='كل زمن يضع الفعل في الماضي أو الحاضر أو المستقبل، ويعطيه واحدًا من أربعة أشكال: <b>simple</b> (الحقيقة المجرّدة)، <b>continuous</b> (جارٍ الآن)، <b>perfect</b> (نظرة إلى الوراء من نقطة لاحقة: الآن، أو لحظة في الماضي أو المستقبل) أو <b>perfect continuous</b> (جارٍ حتى تلك النقطة). ثلاثة أوقات، وأربعة أشكال: اثنا عشر مخيمًا من ثلاثة عشر. كل صيغة أدناه هي &laquo;she&raquo; + CLIMB، والرقم هو مخيمها.',
    gCaption='الأزمنة الاثنا عشر بحسب الوقت والشكل؛ كل واحد منها يقود إلى مخيمه.',
    gGoing='الباقي، المخيم 5، مستقبلٌ تراه قادمًا من الآن.',
    ...
    g3P='تصعد المخيمات بالترتيب الذي ستحتاجها به، لا بترتيب كتب القواعد. <b>المخيمات الأساسية</b> هي الأزمنة التي تستعملها كل يوم؛ و<b>الحافة</b> شائعة لكنها تحتاج إلى عناية أكبر؛ و<b>الهجوم على القمة</b> نادر وأغلبه مكتوب. أربع صيغ للمجهول لا مخيم لها: يمكن بناؤها &mdash; HAS BEEN BEING CLEANED &mdash; لكن لا أحد يقولها.',
    ...
    tailDefault='تسعة من المخيمات تفتح صيغة مجهول؛ <em>used to</em> يُفتح بعد المخيم 2 أو 3، و<em>be used to</em> بعد المخيم 2، والسببية بعد أول محطة لك في النزول.',
    ...
    whyCausative='أنهِ أي محطة في النزول &mdash; السببية مبني للمجهول يعتمر قبعة.',
...
LANGS['zh'] = dict(
    ...
    h1='十三座营地，两面山坡',
    lead='上山用主动语态，按你真正会用到每个时态的频率排列。下山用被动语态：还是这十三座营地，顺序反过来 &mdash; 其中九座有值得学的被动语态。',
    countsAria='路线上有什么', cUp='座上山营地', cDown='个下山被动', cOff='条路线外的课', cFree='节免费',
    ...
    guideBtn='我在看什么？', ascH='上山', ascSub='主动语态 &middot; 从大本营到山顶',
    ...
    lgColour='彩色 &mdash; 点一下就去', lgGrey='灰色 &mdash; 完成它的营地后打开',
    lgShape='圆形 = 主动语态，菱形 = 被动语态', ascList='上山 &middot; 主动语态',
    zFH='大本营', zFSub='天天都用得上', zRH='山脊', zRSub='常用，但需要多加注意',
    ...
    free='免费', level='级别', aboutRoute='关于这条路线', whereH='你现在在哪里',
    darkH='下山路上有四站故意留暗',
    darkP='present perfect continuous、past perfect continuous、future continuous 和 future perfect continuous 的被动语态都能造出来 &mdash; HAS BEEN BEING CLEANED、WILL BE BEING CLEANED &mdash; 但母语者全都避免使用，而是把句子换个说法。那里没什么可学的，所以也就没有营地。',
    teachH='你在教学吗？',
    teachP='地图下方的开关可以在这个浏览器里一次解除所有进度锁 &mdash; 适合课堂大屏幕，或已经爬过一遍的班级。Pro 课程仍然需要 Pro。<a href="block-camp.html">Block Camp</a> 在一个由方块搭成的世界里走过其中九个时态。',
    proP='<strong>四节课免费，无需注册：</strong>营地一和营地二、营地一打开的被动语态，以及 <em>used to</em>。Pro 解锁其余二十一节课，以及网站上的所有其他课程。',
    plans='方案与价格',
    gKick='出发之前', gTitle='你在看什么', gClose='关闭说明',
    g1H='时态 = 时间 + 形态',
    ...
    gGoing='剩下的那一个，营地 5，是一个已经能看到正在到来的将来。',
    g2H='两面山坡：主动与被动',
    ...
    g3H='为什么是这个顺序',
    g3P='营地按照你会需要它们的顺序往上排，而不是语法书的顺序。<b>大本营</b>是你每天都用的时态；<b>山脊</b>很常用，但需要多加注意；<b>冲顶</b>少见，多出现在书面语里。有四个被动语态没有营地：它们造得出来 &mdash; HAS BEEN BEING CLEANED &mdash; 但没人这么说。',
    ...
    again='再爬一次：营地一',
    lockNote='<strong>{c} / 13</strong> 座营地 &middot; <strong>{d} / 9</strong> 个下山站 &mdash; {tail}',
    tailAll='全部已打开。', tailWhole='整座山都已打开，两面都是。',
    tailDefault='九座营地各自打开一个被动语态；<em>used to</em> 在营地 2 或 3 之后打开，<em>be used to</em> 在营地 2 之后，使役结构在你的第一个下山站之后。',
    teachOff='我在教这个 &mdash; 全部打开', teachOn='锁已解除 &mdash; 全部已打开',
    teachWhy='学习者完成对应的营地，就能打开每个被动语态和三节路线外的课。老师和赶时间的人可以一次全部打开。',
    pair='营地 {n}（{tense}）', whyDescent='完成{pair}，这一站就会打开。',
    whyUsedTo='完成营地 2 或营地 3 &mdash; used to 是把 present simple 的习惯搬到了过去。',
    whyBeUsedTo='完成营地 2（present simple）&mdash; be used to 是一种状态，而不是一个事件。',
    whyCausative='完成任意一个下山站 &mdash; 使役结构就是戴了帽子的被动语态。',
    srLocked='（已锁：{why}）', srDone='（已完成）', fromOpen='来自营地 {n}', fromLocked='营地 {n} 之后',
    hintNoCamp='{label}：这里没有营地 &mdash; 请看列表下方的说明。', hintLocked='{label}：{why}',
)

LANGS['ja'] = dict(
    langGroup='言語', sub='時制をのぼっていくルート', eyebrow='ルートマップ',
    h1='13のキャンプ、2つの斜面',
    lead='登りは能動態で、それぞれの時制を実際にどれくらい使うかの順に並んでいます。下りは受動態で、同じ13のキャンプを逆の順にたどります &mdash; そのうち9つには、学ぶ価値のある受動態があります。',
    countsAria='ルートにあるもの', cUp='登りのキャンプ', cDown='下りの受動態', cOff='ルート外', cFree='無料',
    everyStop='すべての地点', mapKick='マップ', mapH2='登りは能動態、下りは受動態',
    mapHow='登りの道に鍵はありません。必要なキャンプから始めてください。下りの各地点は、その元になるキャンプを終えると開きます。それまでは印が灰色です。',
    guideKick='はじめてですか？', guideLine='時制とは何か、能動態と受動態とは何か、そしてキャンプがなぜこの順に並んでいるのかを2分で説明します。',
    guideBtn='何を見ているの？', ascH='登り', ascSub='能動態 &middot; ベースキャンプから山頂へ',
    descH='下り', descSub='受動態 &middot; 山頂からふもとへ',
    ascAria='登り：ベースキャンプから山頂への道に13の色つきのキャンプの印がある山。能動態の時制ごとに1つずつ、そして used to と be used to の2つの雲があります。すべての地点はマップの下にも一覧があります。',
    descAria='下り：夜の同じ山。山頂から下へ、受動態の時制ごとにひし形の印があります。9つはレッスンで、それぞれそのキャンプを終えると開きます。4つは誰も使わない受動態なので暗いままです。月は使役です。すべての地点はマップの下にも一覧があります。',
    lgColour='色つき &mdash; タップで移動', lgGrey='灰色 &mdash; そのキャンプを終えると開く',
    ...
    free='無料', level='レベル', aboutRoute='このルートについて', whereH='今どこにいるか',
    darkH='下りの4つの地点は、わざと暗いままにしています',
    darkP='present perfect continuous、past perfect continuous、future continuous、future perfect continuous の受動態は、作ることはできます &mdash; HAS BEEN BEING CLEANED、WILL BE BEING CLEANED &mdash; が、ネイティブスピーカーはどれも避けて、文を言い換えます。学ぶことが何もないので、そこにはキャンプがありません。',
    teachH='教えていますか？',
    teachP='マップの下のスイッチで、このブラウザーの進行状況の鍵を一度にすべて外せます &mdash; 教室のスクリーン用や、すでに登ったことのあるクラス向けです。Pro のレッスンには引き続き Pro が必要です。<a href="block-camp.html">Block Camp</a> は、ブロックでできた世界でこのうち9つの時制を登ります。',
    proP='<strong>4つのレッスンは無料で、登録も不要です：</strong>キャンプ1と2、キャンプ1が開く受動態、そして <em>used to</em>。Pro では残りの21と、サイトのほかのすべてのレッスンが開きます。',
    plans='プランと料金',
    gKick='登る前に', gTitle='いま見ているもの', gClose='ガイドを閉じる',
    g1H='時制＝時と形',
    g1P='どの時制も、動作を過去・現在・未来のどこかに置き、4つの形のどれかを与えます：<b>simple</b>（そのままの事実）、<b>continuous</b>（進行中）、<b>perfect</b>（あとの時点から振り返る：今、または過去か未来のある時点）、<b>perfect continuous</b>（その時点まで進行中）。3つの時と4つの形で、13のキャンプのうち12になります。下の形はすべて &ldquo;she&rdquo; + CLIMB で、数字はそのキャンプです。',
    gCaption='12の時制を時と形で並べたもの。それぞれ自分のキャンプにつながっています。',
    gGoing='残りの1つ、キャンプ5は、もう来るのが見えている未来です。',
    g2H='2つの斜面：能動態と受動態',
    gAsc='<b>登り &middot; 能動態。</b>動作をする人が先に来ます。',
    gDesc='<b>下り &middot; 受動態。</b>動作を受けるものが先に来て、動作をする人は消えることもあります。',
    g2P='同じ時制を裏返したもの：受動態は、その時制の BE + PAST PARTICIPLE です。だから下りの各地点は登りのキャンプの双子で、そのキャンプを終えると開きます。',
    g3H='なぜこの順番か',
    g3P='キャンプは文法書の順ではなく、あなたが必要になる順に登っていきます。<b>ベースキャンプ</b>は毎日使う時制、<b>稜線</b>はよく使うが注意が必要なもの、<b>山頂アタック</b>はまれで主に書き言葉です。4つの受動態にはキャンプがありません。作れはします &mdash; HAS BEEN BEING CLEANED &mdash; が、誰も言いません。',
    g3Map='マップでは、丸が能動態、ひし形が受動態、灰色はまだ開いていないことを表します。&ldquo;no camp&rdquo; と書かれた薄いひし形は、決して開きません。雲は &ldquo;used to&rdquo; と &ldquo;be used to&rdquo;、月は使役です。',
    startFree='キャンプ1から始める &middot; 無料', backMap='マップに戻る',
    carryOn='続き：キャンプ{n} &middot; {name}', headDown='下りへ：{name}', offRoute='ルート外：{name}',
    again='もう一度登る：キャンプ1',
    lockNote='<strong>13のうち{c}</strong>キャンプ &middot; <strong>9のうち{d}</strong>下りの地点 &mdash; {tail}',
    ...
    teachOff='授業で使う &mdash; すべて開く', teachOn='鍵を解除 &mdash; すべて開いています',
    ...
    whyUsedTo='キャンプ2かキャンプ3を終えてください &mdash; used to は present simple の習慣を過去に移したものです。',
    whyBeUsedTo='キャンプ2（present simple）を終えてください &mdash; be used to は出来事ではなく状態です。',
    whyCausative='下りの地点をどれか1つ終えてください &mdash; 使役は帽子をかぶった受動態です。',
    srLocked='（ロック中：{why}）', srDone='（完了）', fromOpen='キャンプ{n}から', fromLocked='キャンプ{n}のあと',
    hintNoCamp='{label}：ここにはキャンプがありません &mdash; 一覧の下のメモを見てください。', hintLocked='{label}：{why}',
)
