...
sec('teach', B % 'bg22.jpg', 'right', 'top',
    ...
    para([
      ('Active &mdash; the doer goes first',
       ...
       gloss('Activa: primero quien lo hace', 'Aktiv: der T&auml;ter zuerst')),
      ('Passive &mdash; the same stone, now in front',
       ...
       gloss('La piedra es extra&iacute;da por los aldeanos',
             'Der Stein wird von Dorfbewohnern abgebaut')),
      ('And the doer can go',
       [('', '<em class="obj">The stone</em> <em class="aux">is</em> <em class="pp">mined</em>.')],
       gloss('Y quien lo hace puede desaparecer', 'Und der T&auml;ter kann wegfallen')),
...
sec('teach', B % 'bg02.jpg', 'right', 'top',
    ...
    para([
      ('Regular &mdash; add -ed', [('VERB', 'mine &rarr; mined &rarr; <em class="pp">mined</em>'),
       ...
       gloss('Regulares: se a&ntilde;ade -ed', 'Regelm&auml;&szlig;ig: -ed anh&auml;ngen')),
      ('Irregular &mdash; learn the third one',
       ...
       gloss('Irregulares: apr&eacute;ndete la tercera',
...
sec('teach', B % 'bg04.jpg', 'left', 'top',
    ...
    para([
      ('You do not know who',
       ...
       gloss('No sabes qui&eacute;n', 'Man wei&szlig; nicht, wer')),
      ('Everyone already knows who',
       [('', '<em class="obj">Bread</em> <em class="aux">is</em> <em class="pp">baked</em> in the village.')],
       gloss('Todos saben ya qui&eacute;n', 'Alle wissen es ohnehin')),
...
sec('teach', B % 'bg06.jpg', 'right', 'top',
    ...
    para([
      ('Keep &lsquo;by&rsquo; when the doer is news',
       ...
       gloss('Mant&eacute;n &lsquo;by&rsquo; si el autor es noticia',
             '&lsquo;by&rsquo; bleibt, wenn der T&auml;ter neu ist')),
...
sec('teach', B % 'bg07.jpg', 'left', 'top',
    ...
    para([
      ('A process &mdash; step after step',
       ...
       gloss('Un proceso: paso a paso', 'Ein Ablauf: Schritt f&uuml;r Schritt')),
      ('A rule &mdash; who does it does not matter',
       [('', '<em class="obj">The gate</em> <em class="aux">is</em> <em class="pp">closed</em> at sunset.')],
       gloss('Una norma: da igual qui&eacute;n', 'Eine Regel: wer, ist egal')),
...
match(B % 'bg34.jpg', 'left', 'top', 'Match the active to its passive',
      ...
      [('Villagers mine the stone', 'the stone is mined',
        ...
        'la piedra es extra&iacute;da', 'der Stein wird abgebaut'),
       ('Alex locks the gate', 'the gate is locked',
        'Alex cierra la puerta', 'Alex schlie&szlig;t das Tor',
        'la puerta est&aacute; cerrada', 'das Tor wird geschlossen'),
       ('A creeper breaks the wall', 'the wall is broken',
        'Un creeper rompe el muro', 'Ein Creeper zerst&ouml;rt die Mauer',
        'el muro es roto', 'die Mauer wird zerst&ouml;rt'),
       ('They place the torches', 'the torches are placed',
        'Colocan las antorchas', 'Sie setzen die Fackeln',
        'las antorchas se colocan', 'die Fackeln werden gesetzt'),
       ('Steve bakes the bread', 'the bread is baked',
        'Steve hornea el pan', 'Steve b&auml;ckt das Brot',
        'el pan se hornea', 'das Brot wird gebacken'),
       ('Miners find the diamonds', 'the diamonds are found',
        'Los mineros encuentran los diamantes', 'Bergleute finden die Diamanten',
...
gap(1, 2, B % 'bg27.jpg', 'right', 'top', 'Write the participle',
    ...
    [('The wall is ', 'broken', '. <span class="dim">(break)</span>',
      ...
      'El muro est&aacute; ______. (romper)', 'Die Mauer ist ______. (zerst&ouml;ren)'),
     ('The bread is ', 'baked', '. <span class="dim">(bake)</span>',
      'Regular verb, so the third form is just -ed.', 170,
      'El pan est&aacute; ______. (hornear)', 'Das Brot ist ______. (backen)'),
...
gap(2, 2, B % 'bg12.jpg', 'left', 'top', '&lsquo;is&rsquo;, or &lsquo;are&rsquo;?',
    ...
    [('The torches ', 'are', ' placed by villagers.', 'Torches is plural.', 130,
      ...
      'Die Fackeln ______ von Dorfbewohnern gesetzt.'),
     ('The gate ', 'is', ' locked by Alex.',
      'One gate. Alex is the doer and changes nothing.', 130,
      'La puerta ______ cerrada por Alex.', 'Das Tor ______ von Alex geschlossen.'),
     ('The diamonds ', 'are', ' found underground.', 'Diamonds is plural.', 130,
      'Los diamantes ______ encontrados bajo tierra.',
      'Die Diamanten ______ unter der Erde gefunden.')]),
...
order(B % 'bg40.jpg', 'left', 'top', 'the map | is | drawn | by a villager .',
      ...
      'El mapa es dibujado por un aldeano.',
      'Die Karte wird von einem Dorfbewohner gezeichnet.'),
