...
`level-5-the-long-game.md` are the seventy-five units.
  ...
  * **Book 1 is laid out, twice.** The first proof was eight pages a unit
    with the unwritten exercises as "to write" boxes; Innes: *"dont
    understand what the blank pages are nor the white spaces, also there
    should be more dynamic illustration to text compositions like my
    second house style."* Both fixed. `content_book1.py` now holds every
    exercise the spec lacks — matching, gapped text, rule cards, practice,
    gist questions, role cards, the listening script and questions, the
    pronunciation note, the grammar reference — all AI drafts like the
    dialogues. The pages follow HOUSE-STYLE §15: heading full width, the
    picture in a framed column under it, sides alternating by unit, arch
    and narrow variants; only the opener keeps its hero across the top.
    **Six pages a unit**, not eight: measured, six is what the content
    fills to the foot at A1. `print_book.js` now reports any page whose
    content overflows its 297mm (`overflow:hidden` would otherwise hide
    it), and the build is clean. `build_book.py 1` → `book-1.html`, 112
...
to talk, MET 0/7) and said the reward for a right answer could be an
...
is going to rain' can be written into the game and appear briefly."
  ...
  * **`block-camp/village.html`** — the hiker walks the hub's own trail
    ...
    light, the tower gains a lit window, MET n/9 ticks, and a dog follows
...
39 scenes, en/es/de) and asked to merge "the new scenes" into the shipped
Blocula. **The draft is a narrative revision, not an append** — the five new
scenes do not stand up without twenty rewrites to the existing ones, so the
whole revision went in. Anyone diffing this against the old page should
expect that.

`block-camp/dracula-castle-of-if.html`, now generated by
**`lesson-template/build/build_blocula.py`**. That replaces
`rebuild_blocula.py`, which is dead: it read `/root/work/original.html` and
`tr_*.json` from a cloud sandbox that no longer exists, so it could not be
re-run by anybody. The new one is self-contained — text in
`lesson-template/build/blocula/scenes.json` (39 scenes × 12 languages), the
hotspot table and the CSS in the builder, everything else carried over from
the page on disk so the SEO block survives a re-run. It replaces five
regions and is **byte-for-byte idempotent**; verified by building twice and
comparing md5.

### What the continuity edits are for

The new scenes only make sense with them, so do not cherry-pick:

- `dinner` now hands Jonathan a ring of **brass guest keys**. That is why
  `locked_room` fails (they open bedrooms, not the iron west door) and why
  the iron key on Dracula's coat in `stolen_key` is worth stealing.
- Both act-two routes now converge on the new `servant_stair`, which is what
  opens the west door and reaches `shipping_plan` — previously both routes
  jumped straight there with no door and no stair.
- `stolen_clothes` inverts: the coat and hat are **returned, bloodstained**,
  not stolen. `last_chance` depends on him having a coat for the descent.
- `choice3` now happens **halfway down the sheet-rope** on a lower parapet —
  chapel window left, drainpipe right. That is what gives `crypt_chase` and
  `courtyard_escape` somewhere to come back to.

Flow, both routes still exactly seventeen questions and 85 points:
...
the rope scene, which is why the count is unchanged. All six route
combinations were walked in the browser: 17 questions, ending in the
epilogue, every new scene reachable on the right route.

### `check-glosses.js` — new, and it found a shipped defect

Innes: *"You should check the translations for errors too. I found one
today."* A 12-language deck hides translation faults completely — they are
invisible in a diff of `T()` calls and invisible on the slide unless you
switch to that language. So the defect class got a measurement:

```bash
node lesson-template/check-glosses.js block-camp/dracula-castle-of-if.html
```

It flags a gloss carrying another language's script (a Cyrillic word inside
the Japanese), a gloss with no characters of its own language, a long Latin
run inside zh/ja/ru/ar, a blank or missing gloss, and **two answer options
that read identically in some language**. "Identical to English" is advisory
only — BLOCULA is a proper name and the French *CORRECT · +5 POINTS*
genuinely coincides.

Verified against a deliberately broken copy (three injected faults, all
caught) before being trusted, per CLAUDE.md.

**It found one real fault in the live deck**: `wolves` options 0 and 2 had
byte-identical Russian, so a Russian reader chose between two identical
lines. English contrasts *came* (second conditional) with *had come*
(third); Russian carries that on verbal aspect, and now does
(подходили / подошли … тогда). Fixed in `scenes.json`.

It is worth running over the other four RPGs — this session did not.

### Layout: no black boxes

Innes: *"upgrade the text layout — no black boxes — streamline layout and
use translucent shapes … have the text central."* The old panel stacked
opaque plates: a near-solid panel, and inside it another plate per option,
per route card, per rule card, per feedback line. Now:

- The panel is glass — no border, corner radius, `blur(22px)`, and a veil
  mixed from the palette with `color-mix()` rather than raw black, so it
  stays tied to the hero's colours. **78% ink, not the 60% first tried**:
  60% looked better but the story text lost to a moonlit archway on
  `fallen_key`. Legibility wins; it still reads as glass.
- Options and routes are translucent with a hairline; feedback and rule
  notes are an accent rule and no fill at all.
- **The three route-choice scenes are centred** (`choice1` and `choice2`
  were still corner-pinned; `choice3` already was), which is what the RPG
  standard says `center` is for.

### A defect the new scene exposed

An answered question is the one panel that outgrows the frame — feedback and
CONTINUE appear below three options that already filled it. The panel
scrolls, but **nothing was scrolling it**, so CONTINUE sat below the fold
with no affordance. Measured with `scrollHeight - clientHeight`: 39px under
on `vampire_brides`, 55 on `locked_room`, **138 on `crypt_tomb`** — all
pre-existing, all shipped. The builder now patches `displayAnswer` to scroll
the panel to its bottom.

`scrollIntoView` does **not** work here — `.content` is a transformed,
animated flex container and the browser resolves `block:'nearest'` to no
...
## 2026-09-07 — The three FireShield pitches merged into one lesson

**Live on `main` since 2026-09-07.** The three old pages 301 to it, their
catalogue rows are gone, and it has the one card. The "what is left" list
below is closed; it stays as the record of what the merge involved.

`fireshield-pitch.html`, generated by `lesson-template/build/build_fireshield.py`.
The three parts become three acts of one continuous branching roleplay:
Manchester → Leeds → Dublin, 32 nodes, trust carried across all of it.

### The scoring bug, and why the simulator now exists

Parts 2 and 3 each opened by **asking the learner how the previous part
ended** — four buttons, because a separate file cannot know. Merging them
made that pointless, so the acts hand their outcome forward instead.

The obvious way to do that was to re-base the trust meter at each act
boundary to the values the old recap buttons offered (85/60/45, then
90/65/45). It looked faithful to the originals. It was measurably wrong:

    84% of all 737,280 playthroughs reached the best ending,
    and the worst ending could not be reached at all.

Nine checkpoints all pay out positively, the meter clamps at 100, and Act III
therefore opened *already above its own top threshold*. Nothing about this is
visible by playing the lesson a few times — every playthrough just feels
generous.

Outcomes are now scored on **trust earned against trust available**, per act
and overall, which cannot saturate. The bar the learner watches is that same
number (smoothed by a prior of 20 so the first click does not swing it
50→100), so the meter can never disagree with the ending it leads to.

**`tools/simulate_fireshield.js` is that measurement, kept.** It walks every
path, reports the ending distribution plus five plausible-learner archetypes,
and exits non-zero if any ending becomes unreachable or takes >60% of paths.
Run it after changing any trust value or tier threshold. Current state:

    ideal    (best choices, all first-try)  meter 94 -> national distribution
    strong   (best choices, 70% first-try)  meter 77 -> regional trial
    mixed    (middling, 70%)                meter 47 -> modest first order
    weak     (poorest safe, 50%)            meter 18 -> nothing but a meeting
    careless (poorest safe, none)           meter  0 -> nothing but a meeting

### Other defects found in the originals and fixed

- **Checkpoint trust was free.** A wrong answer just sat there until you
  clicked the right one, so every learner banked every checkpoint point and
  the meter measured nothing but the story choices. Now first-attempt only.
- **A single bad click locked out two thirds of the lesson.** The two
  catastrophic branches still end the run — that consequence is the point —
  but now offer a rewind to the decision, which pops the decision off the
  log rather than hand-adding trust back.
- **`trade off` and `reliable` were each taught twice** across the three
  parts. Act II now teaches `liability` and `accountable` in those slots.
- **No activation stage** in any of the three. There is one now.

### Translation scope — worth knowing before you translate any roleplay

EN/DE/ES, and the switcher is built from `UI_I18N` at runtime so a language
short of English's key count is never offered.

**Only the chrome translates.** HOUSE-STYLE §8 says do not translate the
English being taught, and `deck.py`'s `mc()` settles exactly where the line
is: only the eyebrow and the title carry `data-i18n` there — the question
stem, the options and the explanation carry none. So in this lesson the
scene eyebrows, buttons, briefing, trust label and activation *instructions*
translate; the story prose, Alex's choices, the question stems, the options,
the explanations, the ending text and the target-language chips stay English.
They are the B2 reading input the lesson exists to deliver. Translating them
would leave a learner reading Spanish and practising nothing.

### Artwork

Innes uploaded 15 Midjourney PNGs to `FireShield/` (73 MB, 2944x1648 each).
`wrangler.toml` has `[assets] directory = "."`, so **left in the tree they
would all have been deployed**. Six are cropped to the house 2400x1023 and
saved as JPEGs (1.2 MB total), the PNGs are removed from the tree, and
`LibraryCards/fireshield.jpg` is cut from the hero at 1200x512.

The 73 MB is still in `main`'s history (commits a51ef9a, 5aef597, 6d6cbe9).
That is a permanent repo-weight cost unless the history is rewritten, which
is Innes's call, not a session's.

Palette derived mechanically, `extract-palette.py FireShield/hero.jpg --light`;
every contrast row PASSes. The accent lands warm terracotta against blue-grey
artwork, which reads well — no `--accent-hue` rotation needed.
...
3. ~~`tools/seo.py` has not been run.~~ **Done** — from a cloud session, with
   the diff on the four indexes read before committing, and the catalogue row
   written through the Supabase MCP connector, which reaches the database
   even though the sandbox's own HTTP is 403'd at the proxy. So a cloud
   session CAN add a lesson to the catalogue; it is `seo.py`'s urllib call
   that cannot. Mirror the row into `tools/lessons.json` first and the
   fallback then matches the database.
   ~~ With Supabase unreachable (403 at the
...
4. ~~The three old pages are untouched and still live.~~ **Done** — 301s in
   `src/index.js` (`run_worker_first = true`, so the Worker sees the request
   even with the files deleted), rows deleted, files and their three library
   cards removed.
   ~~ No redirects yet, and
...
### Handing files to a cloud session — one route confirmed dead

Chat attachments reach the model as **images in context, never as files on
disk**. Checked `/mnt/attach`, `/mnt/user-data/working` and a filesystem-wide
sweep for recently-modified image files: nothing. So `extract-palette.py` has
no bytes to read and nothing can be committed.

Google Drive is not a way round it either, and now for two measured reasons,
not one:

- `download_file_content` returns **base64 through the model's context** —
  roughly 30k tokens for a 90 KB image. Fifteen images would exhaust the
  context window before any work started.
- Direct fetch is blocked: `drive.google.com` gives `CONNECT tunnel failed,
  403` at the sandbox proxy, and `googleapis.com` answers but demands an API
  key the session does not have.

The Drive connector *can* cheaply **list** Drive (metadata only), so it is
still useful for finding out what exists. It is not a transport for artwork.

**Uploading to the repo remains the only working route for a cloud session,
and running Claude Code locally on erazorhead is the better answer** — a local
session reads Innes's Downloads folder directly and pushes with his own
credentials, with no upload step at all.
...
*(Superseded 2026-09-07: it took the V7 draft — five new scenes, a
narrative revision and a new builder. See the entry at the top.)*
